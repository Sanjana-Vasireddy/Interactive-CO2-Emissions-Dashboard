...
var state, class_keep, idx, svg, legend, data, sortBy;
...
function barchart() {
    ...
    data = store.co2emissions_barchart;

    color.domain(Object.keys(data[0]).filter(function (d) { return d != 'Country' }));

    data.forEach(function (d) {
        var mystate = d.Country; //add to stock code
        var y0 = 0;
    ...
    //.text("Energy PerCapita CO2 Emissions ");
    ...
    state.selectAll("rect")
        ...
        .attr("y", function (d) {
            ...
            if (d.name === "2008") height_diff = 0; //reset for next d.mystate
...
function plotSingle(d) {
    ...
    state.selection('rect')._groups.forEach(function (d, i) {

        //get height and y posn of base bar and selected bar
        var h_keep = d3.select(d[idx]).attr("height");
        var y_keep = d3.select(d[idx]).attr("y");
...
function change() {
    ...
    var x0 = x.domain(data.sort(sortDescending
        ...
        .map(function (d, i) { return d.Country; }))
        .copy();
...
onMounted(() => {
    if (!store.co2emissions_barchart) {
        d3.csv('co2emissions_barchart.csv').then(result => {
            store.co2emissions_barchart = result;
            message.value = `Data was loaded from file, contains ${store.co2emissions_barchart.length} rows`;
            barchart();
        });
    } else {
        barchart();
